    ...
    <div style={containerStyle}>
      ...
      {announcement && (
        <div
          ...
          className={`battle-announcement${announcement.exiting ? ' exiting' : ''}`}
          style={{ color: '#ffffff', bottom: '0.5rem' }}
        >
          <span style={{ color: announcement.nameColor }}>{announcement.charName}</span>
          {announcement.middle}
          {announcement.targetName && <span style={{ color: announcement.targetColor }}>{announcement.targetName}</span>}
        </div>
      )}
